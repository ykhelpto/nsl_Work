...
class Send_heatbeat(threading.Thread):
    ...
    def run(self):
        while (self.isThread):
            # 发送心跳信息
            time.sleep(60)
            videoFrame = self.rtspFrameTool.getFrameData()  # 获取视频流工具类中的视频帧状态,如果视频帧为null 那么就认为是设备离线状态
            data = None
